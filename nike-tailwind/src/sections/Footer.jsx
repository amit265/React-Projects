        ...
        <div className="flex flex-col items-start">
          ...
          <div className="flex items-center gap-5 mt-8">
            {socialMedia.map((icon) => (
              <div
                className="flex font- justify-center items-center w-12 h-12 bg-white rounded-full"
                key={icon.alt}
              >
        ...
        <div className="flex flex-1 justify-between lg:gap-10 gap-20 flex-wrap">
          {footerLinks.map((section) => (
            <div key={section.title}>
              <h4 className="font-serif text-2xl leading-normal font-medium mb-6 text-white">
                {section.title}
              </h4>
              <ul>
                {section.links.map((link) => (
                  <li
                    className="mt-3 font-serif text-base leading-normal text-white hover:text-slate-gray"
                    key={link.name}
                  >
